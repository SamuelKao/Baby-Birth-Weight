...
hist(df$gestation,
  main = "Histogram of gestation", xlab = "gestation", breaks = 15, ,
  freq = FALSE
)
...
df_clean <- na.omit(df)
clean_age <- df[df$age < 50, ]
clean_smoke <- clean_age[clean_age$smoke == 0 | clean_age$smoke == 1, ]
...
If we change the threshold to 105 then more baby would considered as lower wieght, the incidence rate increases.
